...
export default async function QuizHomePage({
  ...
  const recordRow = (rec: (typeof records)[number]) => (
    <div
      key={rec.id}
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <span className="w-[38px] text-right text-sm font-bold text-accent">{rec.score}점</span>
        <Link href={`/quiz/${rec.quizId}/result`} className="text-xs text-stone-500 hover:text-accent">
          해설 보기 →
        </Link>
      </div>
    </div>
    ...
    <>
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <div className="flex flex-col overflow-hidden rounded-xl border border-line bg-white">
          ...
          {records.slice(0, 3).map(recordRow)}
          {records.length > 3 && (
            <details>
              <summary className="cursor-pointer list-none border-t border-line-soft px-5 py-3 text-center text-[12px] text-stone-400 hover:text-stone-600">
                지난 기록 {records.length - 3}개 더 보기
              </summary>
              {records.slice(3).map(recordRow)}
            </details>
          )}
        </div>
      </div>
